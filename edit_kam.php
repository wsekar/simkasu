...
<head>
    ...
    <title>Halaman Edit Kadar Air Minum | Simkasu</title>
    <meta charset="utf-8">
...
<body class="app sidebar-mini">
    ...
    <main class="app-content">
        <div class="container" style="margin-top:20px">
            <h2>Edit Kadar Air Minum</h2>

            <hr>

            <?php
            //jika sudah mendapatkan parameter GET id_ph dari URL
            if (isset($_GET['id_ph'])) {
                //membuat variabel $id_ph untuk menyimpan id_ph dari GET id_phdi URL
                $id_ph = $_GET['id_ph'];

                //query ke database SELECT tabel ph berdasarkan id_ph= $id_ph
                $select = mysqli_query($koneksi, "SELECT * FROM ph WHERE id_ph='$id_ph'") or die(mysqli_error($koneksi));

                //jika hasil query = 0 maka muncul pesan error
                if (mysqli_num_rows($select) == 0) {
                    echo '<div class="alert alert-warning">id_ph tidak ada dalam database.</div>';
                    exit();
                    //jika hasil query > 0
                } else {
            ...
            if (isset($_POST['submit'])) {
                ...
                $ph_before              = $_POST['ph_before'];
                $ph_after               = $_POST['ph_after'];
                $waktu_perbaikan        = $_POST['waktu_perbaikan'];

                $sql = mysqli_query($koneksi, "UPDATE ph SET id_kandang = '$id_kandang',ph_before='$ph_before', ph_after='$ph_after', waktu_perbaikan='$waktu_perbaikan' WHERE id_ph='$id_ph'") or die(mysqli_error($koneksi));

                if ($sql) {
                    echo '<script>alert("Berhasil mengubah data."); document.location="kadar_air_minum.php?id=' . $id_ph . '";</script>';
                } else {
            ...
            <form action="edit_kam.php?id_ph=<?php echo $id_ph; ?>" method="post">
                ...
                <div class="form-group row">
                    <label class="col-sm-2 col-form-label">Ph Before</label>
                    <div class="col-sm-10">
                        <input type="text" name="ph_before" class="form-control" value="<?php echo $data['ph_before']; ?>" required>
                    </div>
                </div>
                <div class="form-group row">
                    <label class="col-sm-2 col-form-label">Ph After</label>
                    <div class="col-sm-10">
                        <input type="text" name="ph_after" class="form-control" value="<?php echo $data['ph_after']; ?>" required>
                    </div>
                </div>
                <div class="form-group row">
                ...
                <div class="form-group row">
                    ...
                    <div class="col-sm-10">
                        ...
                        <a href="kadar_air_minum.php" class="btn btn-warning">KEMBALI</a>
                    </div>
                </div>
            </form>
